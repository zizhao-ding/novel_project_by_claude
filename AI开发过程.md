...
项目初期有完整的 `前端开发规范.md`（760行）和 `技术选型.md`，但缺乏结构化的、可供 AI 辅助开发直接使用的规则和规格体系。为此建立了一套 "Rules + Spec" 体系，并将规范文档精简为 212 行的架构决策文档，技术选型内容整合到 `README.md` 中。
...
| `10_progress_tracking.md` | - | 新增 | 每次进展需更新 spec + memory 进度 |
| `11_confirm_dialog.md` | - | 新增 | 敏感操作必须 ElMessageBox 二次确认 |
...
| `01_auth_register.md` | 204 | 用户注册登录（✅ 已完成） |
| `02_novel_upload.md` | - | 小说上传功能（✅ 已完成） |

### 已有文件修改

| 文件 | 修改内容 |
|------|----------|
| `前端开发规范.md` | 重构为 212 行架构决策文档，代码示例移入 rules，聚焦「为什么这样选」 |
| `技术选型.md` | ⚠️ 后续已删除，内容整合到 `README.md` 和 `CLAUDE.md` |

## 三、设计原则

1. **760行 → 多个 ≤104行 的规则文件**：完整规范文档太长，AI 无法一次性加载。拆分为聚焦领域的小文件，AI 按需加载，每个文件控制在 100 行左右。

2. **编号前缀 `01_` `02_`...**：确保加载顺序和优先级，方便 CLAUDE.md 中引用。后续可递增新增规则。
...
6. **规范文档转型为「为什么」文档**：`前端开发规范.md` 从 760 行精简为 212 行的架构决策文档，聚焦设计理由。「怎么做」的代码示例全部移入 rules。当 rules 与本文档冲突时，以本文档为准。
...
| 文件 | 内容 |
|------|------|
| `01_project-overview.md` | 项目概况、技术栈、目录结构 |
| `02_rules-spec-system.md` | Rules & Spec 体系说明 |
| `03_ngrok-setup.md` | ngrok 内网穿透配置指南 |
| `04_current-progress.md` | 当前开发进度（已完成 / 进行中 / 待开始） |
| `05_frontend-standards-refactor.md` | 规范文档转型记录（760行 → 212行） |
...
详见 [CLAUDE.md](CLAUDE.md) 末尾的约束速查部分。
