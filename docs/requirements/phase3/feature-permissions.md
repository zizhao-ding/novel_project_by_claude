...
> **优先级**: P0（基础设施，所有功能依赖）
> **创建日期**: 2026-06-28
> **最后更新**: 2026-06-28（优化：移除书架阅读限制，纯可见性控制）

## 1. 概述

建立三级角色权限体系，控制用户对小说内容的访问范围。同时为小说增加可见性标签，实现差异化内容分发。新增管理员专属的用户角色管理页面。

**设计原则**：简单实用，不过度设计。**纯可见性控制** —— 一个 `visibility` 字段决定谁能看到、谁能阅读。书架回归"收藏夹"本质，不承担权限职责。
...
| 普通成员 | `member` | 注册默认角色 | 普通读者，可阅读所有公开小说 |
...
> 设计理念：一层 `visibility` 字段解决所有内容分发问题。书架纯粹是收藏夹，不充当"通行证"。
...
- [ ] 普通成员可阅读所有 `public` 小说（无需加入书架）
- [ ] 种子成员可看到和阅读 `public` + `seed` 小说
- [ ] 管理员可看到和阅读全部小说
- [ ] 种子成员+管理员可下载 TXT 原文件
- [ ] 仅有种子成员+管理员可上传小说
- [ ] 管理员用户管理页可查看所有用户列表
