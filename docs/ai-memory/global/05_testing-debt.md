...
| 前端覆盖率 | <5% | **~15%** |
| 后端覆盖率 | <20% | **~35%** |
> **关联层级**: Harness Layer 6 — 验收层

## 现状

Harness 架构规范 POST-003 要求：
...
**Why:** Harness Layer 6 验收层要求测试覆盖率达标，当前测试严重不足，是最大的架构合规缺口。

**How to apply:** 每次新功能开发时，同步补齐相关模块的测试。在测试覆盖率达到标准前，POST-003 检查应为警告而非阻断。

**相关记忆**: [[03_current-progress]] [[04_harness-architecture]] [[06_architecture-decisions]]
